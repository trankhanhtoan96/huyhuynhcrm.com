...
<article id="post-<?php the_ID(); ?>" <?php post_class($post_classes); ?>>
	...
	<?php
		...
		if (isset($alia_content_width) && "" != $alia_content_width) {
			...
			} else if ($alia_content_width == "grid" || $alia_content_width == 'two_coloumns_list') {
			...
			if ( ($alia_content_width == 'two_coloumns_list' || $alia_content_width == 'grid' || $alia_content_width == 'grid3col' ) && has_post_thumbnail() ):
				...
				if ( !empty( $video ) ) {
					echo '<figure class="post_banner">';
						echo '<div class="entry-video">'.$video[0].'</div>';
					echo '</figure>';
					$post_banner_class = 'has_post_banner';
				}else {

					preg_match_all('#<([a-z]+).+?(?:id|class)\s*=\s*("|\')(fb-video)\2[^>]*>(.+?)</\1>#is', $content, $match);
					if (!empty($match[0][0])) {
						echo '<figure class="post_banner">'.$match[0][0].'</figure>';
						$post_banner_class = 'has_post_banner';
					}
		...
		<div class="post_info_wrapper">
			<?php if ( is_single(get_the_ID()) ||
				...
				echo '<div class="entry-summary blog_post_text blog_post_description">';
					if ($alia_content_width == 'two_coloumns_list' || $alia_content_width == 'grid' || $alia_content_width == 'grid3col') {
						echo alia_excerpt(20);
					}else{
						if (!isset($alia_content_layout) || $alia_content_layout != 'layout_with_sidebar') {
