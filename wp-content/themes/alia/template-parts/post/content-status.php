...
<article id="post-<?php the_ID(); ?>" <?php post_class($post_classes); ?>>
	...
	<?php
		...
		preg_match_all('#<div class="(.+?)(alia_embed_wrapper|twitter_widget_border)"><!--ALIA start embed content-->(.+?)<!--AliA end embed content--></div>#is', $content, $match);
		if (empty($match[0][0])) {
			preg_match_all('#((<a(.+?)class=("|\')gie-single("|\')(.+?)></script>)|(<div class="getty embed(.+?)"((|(.+?))</div>){3}))#is', $content, $match);
			if (!empty($match[0][0])) {
				$match[0][0] = '<div class="getty_widget_wrapper">'.$match[0][0].'</div>';
			}
		}

		// preg_match_all('#<div[^>]*>(.+?)</div>#is', $content, $match);

		// preg_match_all('#<([a-z]+).+?(?:id|class)\s*=\s*("|\')(fb-post|twitter-tweet|fb-video|instagram_embed_wrapper)\2[^>]*>(.+?)</\1>#is', $content, $match);
	?>
	...
	<?php
		...
		if (isset($alia_content_width) && "" != $alia_content_width) {
			...
			} elseif ($alia_content_width == "grid" || $alia_content_width == 'two_coloumns_list' || $alia_content_width == 'grid3col') {
		...
		if ( !is_single(get_the_ID()) && !empty($match[0][0]) ) {
			...
			if ( ($alia_content_width == 'two_coloumns_list' || $alia_content_width == 'grid' || $alia_content_width == 'grid3col' ) && has_post_thumbnail() ):
				?>
